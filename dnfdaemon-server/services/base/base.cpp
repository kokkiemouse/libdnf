...
void Base::dbus_register() {
    ...
    dbus_object->registerMethod(dnfdaemon::INTERFACE_BASE, "read_all_repos", "", "", [this](sdbus::MethodCall call) -> void {
        this->read_all_repos(call);
    });
    dbus_object->registerSignal(dnfdaemon::INTERFACE_BASE, dnfdaemon::SIGNAL_REPOSITORIES_READY, "b");
...
void Base::read_all_repos(sdbus::MethodCall call) {
    ...
    std::thread([this]() {
        ...
        auto signal = dbus_object->createSignal(dnfdaemon::INTERFACE_BASE, dnfdaemon::SIGNAL_REPOSITORIES_READY);
        signal << true;
        dbus_object->emitSignal(signal);
    }).detach();
}
